<template>
  <v-div>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>bus information manegment </v-toolbar-title>
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editorganization.bus_Id"
                        label="bus id"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
                      <v-select
                        v-model="editorganization.transaction_Machine"
                        :items="T_Machine"
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-select
                        v-model="editorganization.ticket_Machine"
                        :items="tt"
...
import organizationService from "./../../service/BusService";
export default {
  data: () => ({
    org: null,
    search: "",
    ...
    organization_header: [
      ...
      { text: "transaction machine", value: "transaction_Machine" },
      { text: "ticket machine ", value: "ticket_Machine", sortable: false },
    ...
    "$route.query.search": {
      ...
      async handler(value) {
        try {
          ...
          if (value) {
            const fetchedData = (
              await organizationService.displayBus(value, token)
            ).data.message;
  ...
  async mounted() {
    try {
      ...
      const org_Data = (await organizationService.displayAllBus(token)).data
        .message;

      for (let i in org_Data) {
        var each__Organization = {
          bus_Id: org_Data[i].bus_Id,
          transaction_Machine: org_Data[i].transaction_Machine.machine_Id,
          ticket_Machine: org_Data[i].ticket_Machine.machine_Id,
          status: org_Data[i].status,
          _id: org_Data[i]._id,
        };
        this.organization.push(each__Organization);
      }
      const fetchedData = (await Machine.displayAllMachine(token)).data.message;
      for (var i in fetchedData) {
        if (fetchedData[i].machine_Type == "Transaction") {
          this.T_Machine.push(fetchedData[i].machine_Id);
        }
        if (fetchedData[i].machine_Type == "Ticket")
          this.tt.push(fetchedData[i].machine_Id);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  methods: {
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        const token = this.$store.state.token;
        this.resp = await organizationService.update(data, oi, token);
      } else {
        console.log("register new branch");
        console.log(this.editorganization);
        const token = this.$store.state.token;
        const credential = this.editorganization;
        this.resp = await organizationService.addBus(credential, token);
...
//           this.search = value;
//           const token = this.$store.state.token;
//           const fetchedData = (await bus.displayBus(value, token)).data.message;
