<template>
  <div>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>organization information manegment </v-toolbar-title>
          <v-spacer></v-spacer>
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editorganization.organization_Id"
                        label="organization id"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editorganization.about_Organization"
...
export default {
  data: () => ({
    ...
    resp: "",
    organization_header: [
      { text: "organization id", value: "organization_Id", sortable: false },
      {
      ...
      { text: "main office", value: "main_Office", sortable: false },
    ...
    "$route.query.search": {
      ...
      async handler(value) {
        try {
          ...
          if (value) {
            const fetchedData = (
              await organizationService.displayOrganization(value, token)
            ).data.message;
  ...
  methods: {
    async display() {
      try {
        ...
        const org_Data = (
          await organizationService.displayAllOrganization(token)
        ).data.message;
        console.log(org_Data);

        for (let i in org_Data) {
          var each__Organization = {
            organization_Id: org_Data[i].organization_Id,
            organization_Name: org_Data[i].organization_Name,
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.resp = await organizationService.addOrganization(
          credential,
          token
        );
