                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editorganization.branch_Id"
                        label="brancch id"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
...
import organizationService from "./../../../service/BranchService";
export default {
  data: () => ({
    org: null,
    search: "",
    ...
    "$route.query.search": {
      ...
      async handler(value) {
        try {
          ...
          if (value) {
            const fetchedData = (
              await organizationService.displayBranch(value, token)
            ).data.message;
  ...
  async mounted() {
    try {
      ...
      const org_Data = (await organizationService.displayAllBranch(token)).data
        .message;

      for (let i in org_Data) {
        var each__Organization = {
          branch_Id: org_Data[i].branch_Id,
          branch_Name: org_Data[i].branch_Name,
          // organization_Type: org_Data[i].organization_Type,
          address: org_Data[i].address,
  ...
  methods: {
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.resp = await organizationService.addBranch(credential, token);
