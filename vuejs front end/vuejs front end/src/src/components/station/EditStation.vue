          ...
          <v-card elevation="0" class="ma-4">
            ...
            <v-card-text>
              <v-text-field
                label="station  id"
                :rules="inputRules"
                v-model="si.station_Id"
              ></v-text-field>
              <v-text-field
                label="station name"
                :rules="inputRules"
                v-model="si.station_Name"
              ></v-text-field>
              <!-- <v-text-field
                label="latitude"
                dense
                :rules="inputRules"
                v-model="si.location.latitude"
              ></v-text-field>
              <v-text-field
                label="longtitude"
                :rules="inputRules"
                v-model="si.location.longitude"
              ></v-text-field> -->
...
export default {
  ...
  watch: {
    "$route.query.search": {
      ...
      async handler(value) {
        ...
        if (value) {
          this.si = (await StationService.show2(value, token)).data.message;
        }
      },
    },
  },
  async mounted() {
    const token = this.$store.state.token;
    try {
      const station_Name = this.$store.state.route.params.station_Name;
      if (station_Name) {
        const fatchedData = (await StationService.show2(station_Name, token))
          .data.message;
