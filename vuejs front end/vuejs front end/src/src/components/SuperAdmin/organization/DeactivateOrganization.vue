    ...
    <v-content class="grey lighten-3">
      ...
      <v-card elevation="0" class="ma-4" min-width="400">
        ...
        <div v-for="(employee, index) in EmpInfo" :key="index">
          <v-card-text style="line-height: 1">
            org name:{{ employee.organization_Name }}</v-card-text
          >
          <v-card-text> org id:{{ employee.organization_Id }}</v-card-text>
          <v-card-text>org type:{{ employee.organization_Type }}</v-card-text>
          <v-card-text>phone number:{{ employee.phone_Number }}</v-card-text>
...
<script>
import EmployeeService from "./../../../service/OrganizationService";
  ...
  methods: {
    async deactivate_Account(object_id) {
      try {
        ...
        const token = this.$store.state.token;
        await EmployeeService.deactivate_Credential(object_id, token);
