    ...
    <v-content class="grey lighten-3">
      ...
      <v-card elevation="0" class="ma-4" min-width="400">
        ...
        <div v-for="(employee, index) in EmployeeInformation" :key="index">
          <v-card-subtitle>name:{{ employee.branch_Name }}</v-card-subtitle>
          <v-card-text>phone number:{{ employee.phone_Number }}</v-card-text>
          <v-card-text>branch id:{{ employee.branch_Id }}</v-card-text>
...
<script>
import bs from "./../../../service/BranchService";
  ...
  methods: {
    async Activate_Customer(oi) {
      ...
      try {
        ...
        const token = this.$store.state.token;
        await bs.activate_Credential(oi, token);
      } catch (err) {
  ...
  watch: {
    "$route.query.search": {
      ...
      async handler(value) {
        this.search = value;
        const token = this.$store.state.token;
        const emp = (await bs.show2(value, token)).data.message;
