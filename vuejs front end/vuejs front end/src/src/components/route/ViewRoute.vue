                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editorganization.route_Id"
                        label="route id"
                      ></v-text-field>
                    </v-col>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editorganization.route_Name"
                        label="route name"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
...
import organizationService from "./../../service/RouteService";
export default {
  data: () => ({
    org: null,
    search: "",
    ...
    organization_header: [
      ...
      { text: "route name", value: "route_Name", align: "start" },
    ...
    "$route.query.search": {
      ...
      async handler(value) {
        try {
          ...
          if (value) {
            const fetchedData = (
              await organizationService.displayRoute(value, token)
            ).data.message;
  ...
  async mounted() {
    try {
      ...
      const org_Data = (await organizationService.displayAllRoute(token)).data
        .message;
      console.log(org_Data);

      for (let i in org_Data) {
        var each__Organization = {
          route_Id: org_Data[i].route_Id,
          route_Name: org_Data[i].route_Name,
          station: org_Data[i].station,
  ...
  methods: {
    ...
    async activateConfirm() {
      ...
      await organizationService.activate_Route(
    ...
    async deactivateConfirm() {
      ...
      const token = this.$store.state.token;
      await organizationService.deactivate_Route(
        this.organization[this.editedIndex]._id,
        token
      );
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.resp = await organizationService.AddRoute(credential, token);
