...
export default {
  ...
  async mounted() {
    ...
    var sum = 0;
    data = (
      await display_chart.machine_analysis({
        type: "Machine_By_Machine_Type",
        output: { type: "other" },
      })
    ).data.message;
  //  console.log(data)
    // var Mobile_number =
    //   data.find(
    //     (element) => JSON.stringify(element._id) == JSON.stringify([Type[0]])
    //   ) || 0;
    // var Card_number =
    //   data.find(
    //     (element) => JSON.stringify(element._id) == JSON.stringify([Type[1]])
    //   ) || 0;
    ...
    this.renderChart(
      {
        ...
        datasets: [
          {
            backgroundColor: data.backgroundColor, //[this.gradient, this.gradient2, "#00D8FF"],
            data: data.count,
          },
        ],
