                                        ...
                                        if(Transaction_Object.ticket_Type == 'Ticket') {
                                                ...
                                                if(auth_ticket.success)
                                                  { 
                                                    ...
                                                    Transaction_Object.customer = auth_ticket.message.customer;
                                                  }
                                                  else  
    ...
    async view_Transaction(req,res, limit = 5 , select = 'transaction_Id transaction_Amount entry_Station exit_Station'){
       try{
            if(req.user.customer){
               var result = await this.search_For_Transaction_IV({customer: req.user.customer})
              ...
              var customer = await Customer.search_For_Customer_EV(req , req.query);
              if(customer.success){
                if(await Auth.authenticate_Access({customer: customer.message.customer_type, viewer: true},req.user.employee)){
                       
                      var result = await this.search_For_Transaction_IV({customer: customer.message._id})  
                        ...
                        if(result.success)
                          ...
                          return await Respond.respond(req, res , {success: false , message: 'No Customer or No transaction could be found'})
                             ...
                             return await Respond.respond({success: false , message: "attempt to view customer transaction with out access"});          
                        }
                      else
