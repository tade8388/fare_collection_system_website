...
async search_for_Ticket_IV(data , limit = 1, populate = '' , sort=''){
  try{
    ...
    if(populate == '')
    populate =[ {path: 'customer' , select: '_id name customer_Id'} , {path: 'entry_Station' , select: '_id station_Id station_Name geometry'},{path: 'machine' , select: '_id machine_Id ip_Address status'},{path: 'entry_Station' , select: '_id station_Id station_Name geometry'}]  
    if(sort == '')
     sort = '-1'
